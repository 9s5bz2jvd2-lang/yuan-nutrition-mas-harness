# LingTai Simple v0.12 Implementation Report

## Summary

v0.12 continues the “真实验收模式” work after v0.11. v0.11 wrote real internal-mailbox dispatches into the LingTai network; v0.12 closes the first feedback loop: Simple can now collect real internal-mail replies from a configured reply inbox and mark the corresponding dispatch/task as replied.

v0.12 also adds a confirmation-gated lifecycle surface for existing real LingTai agents. It does not execute destructive lifecycle operations immediately. It creates a preview item in the approval queue, then applies the real `.sleep` / `.suspend` / `.interrupt` / `.clear` signal or CPR process launch only after approval.

## Changed files

- `server.py`
  - Bumps visible runtime strings to v0.12.
...
- `static/app.js`
  - Updates frontend strings to v0.12.
...
- `static/index.html`
  - Updates the visible milestone text from v0.11 dispatch-only to v0.12 dispatch + reply collection + lifecycle gate.
...
```text
OK LingTai Simple v0.12 self-check passed
```
...
- v0.12 is still not a full avatar manager: no real spawn/delete UI, no nirvana, no arbitrary filesystem deletion.
