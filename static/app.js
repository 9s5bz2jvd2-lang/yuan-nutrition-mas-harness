/* 圆酱专属轻量版灵台 v0.16 — 前端逻辑（纯原生 JS，无依赖） */
...
function openWechatModal() {
  openModal("💬 微信入口任务 / 桥接测试", `
    <div class="preview">v0.16 已接入真实微信桥接端点：实际运行时由当前 LingTai WeChat MCP 把圆酱微信消息写入本服务，再原路回复；这里仍可手动提交一条本地测试消息。</div>
...
async function openLingTaiRuntimeModal(taskId = '', presetAddress = '') {
  ...
  openModal("📮 派到真实 LingTai agent（内部邮箱）", `
    <div class="preview">v0.16 真实能力：把任务写入 <code>.lingtai/&lt;sender&gt;/mailbox/outbox</code>，由 kernel mailman 投递给真实 agent。不是 mock；会唤醒/占用真实 agent。</div>
    <label>选择本地任务</label>
...
async function openLingTaiAvatarModal() {
  ...
  openModal('🧬 创建真实 LingTai avatar', `
    <div class="preview">v0.16 真实能力：确认后会在同一个 .lingtai 网络下创建 peer agent 目录，复制并净化模板 init.json，写入 .prompt，然后启动 lingtai-agent run。先只开放 shallow；不继承微信/Telegram/IMAP addon，避免重复 poller；不会删除任何既有 agent。</div>
...
async function openLingTaiBindModal() {
  ...
  openModal('🔗 绑定既有真实 LingTai agent', `
    <div class="preview">v0.16 真实管理能力：把同网已有真实 agent 绑定成 Simple 本地卡片，方便派发/回收/生命周期管理。此操作只改 Simple 本地状态，不启动、不删除真实 agent。</div>
    <label>真实 LingTai agent</label>
...
async function openLingTaiRetireModal() {
  ...
  openModal('🌙 退休/解绑真实 avatar（不删除目录）', `
    <div class="preview">v0.16 安全语义：退休/解绑不会删除真实 agent 目录，不会 nirvana；只把 Simple 本地卡片标为已退休。可选写入 sleep/suspend signal，让真实 agent 入睡或停止进程。</div>
    <label>已绑定本地卡片</label>
