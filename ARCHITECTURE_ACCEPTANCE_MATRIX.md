...
| A03 | Task Router / Orchestrator | Partial | `/api/task/route` 统一分类普通任务、多 agent、洞察、心流、收功、LingTai mailbox dispatch/collect、Claude/Codex handoff、daemon plan；敏感任务进确认队列。 | v0.23 已补 lightweight LingTai harness run：每条输入形成 intake/route/approval/dispatch/collect/return 审计链，并要求 controller 回信 HARNESS_REPLY_JSON 结构化结果；Simple 仍不直接启动 daemon/Codex/Claude/avatar，也不绕过既有安全纪律。 | `python3 scripts/self_check.py` 覆盖 `/api/task/route`、harness_run_id、WeChat 默认 route_id、fake mailbox dispatch、worker HARNESS_REPLY_JSON 回收和 pending outbox。 |
...
| A05 | Model/API Registry | Partial | Provider catalog 含 GPT/OpenAI-compatible、MiMo、DeepSeek、MiniMax、GLM、自定义；Keychain/env/.secrets 受限 fallback 存/取 key；`/api/model/test` 对兼容端点真实调用，需 `confirm_cost`；v0.20 已加 `/api/cost/status`、`/api/cost/policy`、本地价格表、provider 单次 cap、日 cap、任务 cap 与预算预检，越线先生成 `budget_override`。 | MiMo/MiniMax 端点需用户填兼容 base_url；预算/成本仍是本地估算，不连接供应商真实账单/余额，默认价格表需按实际模型校准。 | `python3 scripts/self_check.py` 验证未确认费用拒绝、预算越线确认闸、key 不落盘；真实模型测试需显式确认。 |
...
| A07 | Approval Queue | Partial | rollback、delete_agent、sensitive task、Claude L3/L4/L5、LingTai lifecycle/avatar、PR/merge 等进确认队列；UI 显示预览；v0.22 新增 scoped grants：可对 grantable 非破坏动作选择 allow-once / allow-for-task，后续匹配项 `grant自动确认` 并记录 `grant_id/used_by/remaining_uses`。 | 破坏性动作仍必须逐项确认；日志/截图/报告外发确认未单独做；未越线的模型调用仍使用 `confirm_cost` checkbox，越线才生成队列项。 | `python3 scripts/self_check.py` 覆盖 allow-once 自动确认与 `code_merge` 拒绝 grant；隔离 rollback/commit smoke。 |
...
1. **受控 worker 直连执行深化**：v0.23 已完成 controller mailbox + HARNESS_REPLY_JSON 结构化回信汇总；下一步若要更进一步，可让 controller 侧形成标准执行协议/模板，减少人工解释。
